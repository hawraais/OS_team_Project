...
#include "freertos/event_groups.h"  // Add this with other includes

#define VOICE_EVENT_BIT  (1 << 0)  // Event bit for voice task
...
void app_main(void)
{
   init_mutex();
    ...
    // Initialize Event Group and Create Task
    voice_event_group = xEventGroupCreate();
    if (voice_event_group == NULL) {
        ESP_LOGE("EventGroup", "Failed to create voice_event_group!");
        return; // Abort initialization if the event group is not created
}

    xTaskCreate(voice_announcement_task, "VoiceTask", 2048, NULL, 5, NULL);

    // Example test: Trigger an initial brightness level
