...
esp_err_t audio_handle_info(PDM_SOUND_TYPE voice)
{
    ...
    case SOUND_TYPE_FACTORY:
        ...
        break;
    case SOUND_TYPE_0:
        sprintf(filepath, "%s/%s", CONFIG_BSP_SPIFFS_MOUNT_POINT, "audio_0.mp3");
        break;
    case SOUND_TYPE_25:
        sprintf(filepath, "%s/%s", CONFIG_BSP_SPIFFS_MOUNT_POINT, "audio_25.mp3");
        break;
    case SOUND_TYPE_50:
        sprintf(filepath, "%s/%s", CONFIG_BSP_SPIFFS_MOUNT_POINT, "audio_50.mp3");
        break;
    case SOUND_TYPE_75:
        sprintf(filepath, "%s/%s", CONFIG_BSP_SPIFFS_MOUNT_POINT, "audio_75.mp3");
        break;
    case SOUND_TYPE_100:
        sprintf(filepath, "%s/%s", CONFIG_BSP_SPIFFS_MOUNT_POINT, "audio_100.mp3");
        break;
    }
...
esp_err_t app_audio_play_file(const char *file_path) {
    // Implementation for playing an audio file
    printf("Playing audio file: %s\n", file_path);
    return ESP_OK;
}

esp_err_t app_audio_tts_play(const char *text) {
    // Implementation for playing TTS text
    printf("Playing TTS: %s\n", text);
    return ESP_OK;
}